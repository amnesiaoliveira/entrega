...
- **Fluxo:** pendente → em rota → entregue. É obrigatório atribuir um entregador para iniciar a rota. Pendentes e entregas em rota podem ser editadas ou canceladas. Entregues e canceladas permanecem no histórico, sem edição.
- **Detalhes:** histórico com autor e horário, ligação para o cliente, link do endereço no Google Maps, edição e mudança de status. O mapa abre uma busca pelo endereço; não há rastreamento GPS.
- **Impressão:** usa a ficha fornecida em `template_controle_entrega.html`, em A5 paisagem (meia folha A4), preenchida pelo sistema. Desative cabeçalhos e rodapés do navegador e use escala de 100%. Textos muito longos podem precisar de páginas adicionais para não serem cortados.
